...
```
① 원본 입력 토큰 계산      →  countTokens (전송 전)
② 입력 최적화             →  압축 / 리사이징 / 무음 제거 / 형식 제한
③ 최적화 입력 토큰 계산    →  countTokens (전송 전)
④ 원본·최적화 요청 각각 실행 →  Gemini 응답의 실제 usageMetadata 수집
⑤ 감소율 = (원본 − 최적화) / 원본 × 100   →  DB 저장 + 차트 표시
```
